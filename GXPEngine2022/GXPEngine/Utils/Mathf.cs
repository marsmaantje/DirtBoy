...
namespace GXPEngine
{
	...
	public static class Mathf
	{
		...
		/// <summary>
		/// Repeats value between range
		/// </summary>
		/// <param name="value">value to repeat</param>
		/// <param name="min">lower bounds</param>
		/// <param name="max">upper bounds</param>
		/// <returns>value in between bounds</returns>
		public static float Repeat(float value, float min, float max)
        {
            float range = max - min;
			float returnVal = value - min;
			returnVal -= Mathf.Floor(returnVal / range) * range;
			return returnVal + min;
        }

		/// <summary>
		/// Bounces the value between the bounds
		/// </summary>
		/// <param name="value">Value to bounce</param>
		/// <param name="min">Lower bounds</param>
		/// <param name="max">Upper bounds</param>
		/// <returns>value in between bounds</returns>
		public static float PingPong(float value, float min, float max)
        {
			...
			float returnVal = value - min;
			returnVal -= Mathf.Floor(returnVal / range) * range;
			if(Mathf.Floor((value - min) / range) % 2 != ((value - min) < 0 ? -1 : 0))
            {
				returnVal = range - returnVal;
            }
			return returnVal + min;
		}

		public static bool RoughlyEquals(float a, float b, float epsilon)
        {
